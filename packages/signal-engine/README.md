...
**CUSUM change point** (`cusum`). Two-sided tabular CUSUM in reference-sigma units with slack `k` (0.5) and decision interval `h` (5). The change point is where the winning sum last left zero, and `startedAt` is that timestamp. The evidence threshold is the mean the post-change segment needed to reach, which falls out of `S = m(d - k) > h`. Severity is the size of the shift in reference sigmas (0 at `k`, 1 at `severityShiftSigmas`), not the accumulated sum, so a tiny shift held for a week is a certain but low-severity signal. `h = 5` gives an in-control run length of roughly 465 iid samples; hourly weather is autocorrelated and wants `h` of 8 or more. Good for step changes that persist: a pressure regime change, a Bz rotation.
