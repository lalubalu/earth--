...
export function EventHistogram({ signal, events, now, width = 640, height = 220 }: HistogramProps) {
  const model = useMemo(() => {
    ...
    const x = scaleTime().domain([new Date(from), new Date(now)]).range([MARGIN.left, width - MARGIN.right]);
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" role="img" aria-label="Event counts per bin against the expected rate">
      {y.ticks(3).map((v) => (
        <g key={v}>
          <line x1={MARGIN.left} x2={width - MARGIN.right} y1={y(v)} y2={y(v)} stroke="var(--color-line)" />
          <text x={MARGIN.left - 6} y={y(v)} dy="0.32em" textAnchor="end" fontSize={10} fill="var(--color-ink-3)" fontFamily="var(--font-mono)">
            {formatAxisValue(v)}
          </text>
        </g>
      ))}
      {x.ticks(width > 480 ? 6 : 4).map((d) => (
        <text key={d.getTime()} x={x(d)} y={bottomY + 14} textAnchor="middle" fontSize={10} fill="var(--color-ink-3)" fontFamily="var(--font-mono)">
          {formatTick(d, model.span)}
        </text>
      ))}
      <rect x={recentX} y={MARGIN.top} width={width - MARGIN.right - recentX} height={bottomY - MARGIN.top} fill="var(--color-accent)" opacity={0.07} />
      ...
      <line x1={MARGIN.left} x2={width - MARGIN.right} y1={y(model.expectedPerBin)} y2={y(model.expectedPerBin)} stroke="var(--color-accent-2)" strokeDasharray="4 3" />
      <text x={width - MARGIN.right} y={y(model.expectedPerBin) - 4} textAnchor="end" fontSize={10} fill="var(--color-accent-2)" fontFamily="var(--font-mono)">
        expected {formatAxisValue(model.expectedPerBin)} per {model.binMs >= DAY ? 'day' : `${model.binMs / HOUR} h`}
      </text>
    </svg>
  );
}
...
export function MagnitudeScale({ signal, width = 640 }: MagnitudeScaleProps) {
  ...
  const axisY = 44;
  const marks = [
    { label: `baseline median ${formatAxisValue(baseline)}`, v: baseline, color: 'var(--color-ink-3)', dy: -18 },
    { label: `threshold ${formatAxisValue(threshold)}`, v: threshold, color: 'var(--color-accent-2)', dy: 18 },
    { label: `observed ${formatAxisValue(observed)}`, v: observed, color: 'var(--color-accent)', dy: -18 },
  ];
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" role="img" aria-label="Observed magnitude against the rule threshold and the baseline median">
      <line x1={MARGIN.left} x2={width - MARGIN.right} y1={axisY} y2={axisY} stroke="var(--color-line-2)" />
      <rect x={x(threshold)} y={axisY - 3} width={Math.max(0, x(max) - x(threshold))} height={6} fill="var(--color-accent)" opacity={0.15} />
      {x.ticks(6).map((v) => (
        <text key={v} x={x(v)} y={axisY + 16} textAnchor="middle" fontSize={10} fill="var(--color-ink-3)" fontFamily="var(--font-mono)">
          {formatAxisValue(v)}
        </text>
      ))}
      {marks.map((m) => (
        <g key={m.label}>
          <line x1={x(m.v)} x2={x(m.v)} y1={axisY - 8} y2={axisY + 8} stroke={m.color} strokeWidth={2} />
          <text x={x(m.v)} y={axisY + m.dy} textAnchor="middle" fontSize={10} fill={m.color} fontFamily="var(--font-mono)">
            {m.label}
          </text>
        </g>
      ))}
