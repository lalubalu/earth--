...
const ONE_MINUTE_FEED: SeriesOverrides = {
  ...
  cusum: { windowMs: DAY, decision: 60 },
};
...
const KP_1M: SeriesOverrides = {
  ...
  cusum: { windowMs: 6 * HOUR, decision: 60 },
};
...
const FORECAST_META: Record<(typeof FORECAST_VARS)[keyof typeof FORECAST_VARS], CityVar> = {
  temp: { suffix: 'temp', label: 'temperature', unit: '°C', minSigma: 0.5 },
  pressure: { suffix: 'pressure', label: 'sea-level pressure', unit: 'hPa', minSigma: 0.5 },
  gust: { suffix: 'gust', label: 'wind gusts', unit: 'km/h', minSigma: 3 },
  precip: { suffix: 'precip', label: 'precipitation', unit: 'mm', minSigma: 0.5 },
};

const AIR_META: Record<(typeof AIR_VARS)[keyof typeof AIR_VARS], CityVar> = {
  pm25: { suffix: 'pm25', label: 'PM2.5', unit: 'μg/m³', minSigma: 2 },
  aqi: { suffix: 'aqi', label: 'US AQI', unit: '', minSigma: 3 },
};
...
export const DESCRIPTORS: readonly SeriesDescriptor[] = [
  { id: NOAA_SERIES.speed, label: 'Solar wind speed', unit: 'km/s', minSigma: 5, overrides: ONE_MINUTE_FEED },
  { id: NOAA_SERIES.density, label: 'Solar wind density', unit: 'p/cm³', minSigma: 0.3, overrides: ONE_MINUTE_FEED },
  { id: NOAA_SERIES.bz, label: 'Bz (GSM)', unit: 'nT', minSigma: 0.5, overrides: ONE_MINUTE_FEED },
  { id: NOAA_SERIES.kp1m, label: 'Kp (1-min estimate)', unit: '', minSigma: 0.2, overrides: KP_1M },
  { id: NOAA_SERIES.kp3h, label: 'Kp (3-hour)', unit: '', minSigma: 0.3, overrides: KP_3H },
  ...cityDescriptors(),
];
